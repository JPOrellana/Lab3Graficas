...
class Raytracer(object):
    ...
    def rtRayColor(self, intercept, rayDirection, recursion = 0):
        ...
        if material.matType == OPAQUE:
            ...
            for light in self.lights:
                if light.lightType == "Ambient":
                    ...
                    if light.lightType == "Directional":
                        ...
                        lightDir = np.custom_subtract_vectors(light.point, intercept.point)

                        lightDir_norm = np.custom_linalg_norm(lightDir)
                        if lightDir_norm != 0:
                            lightDir = np.custom_multiply_vector_by_scalar(lightDir, 1 / lightDir_norm)
            ...
            outside = np.custom_dot_product(rayDirection, intercept.normal) < 0
            bias = np.custom_multiply_vector_by_scalar(intercept.normal , 0.001)

            rayDirection_negado = [ i * -1 for i in rayDirection]
            reflect = reflectVector(intercept.normal, rayDirection_negado )
            
            if outside:
                reflectOrig = np.custom_add_vectors(intercept.point, bias)
            else:
                reflectOrig = np.custom_subtract_vectors(intercept.point, bias)
            ...
            if not totalInternalReflection(intercept.normal, rayDirection, 1.0, material.ior):
                ...
                if outside:
                    refractOrig = np.custom_subtract_vectors(intercept.point, bias)
                else:
                 refractOrig = np.custom_add_vectors(intercept.point, bias)
                ...
                reflectColor = np.custom_multiply_vector_by_scalar(reflectColor, Kr)
                refractColor = np.custom_multiply_vector_by_scalar(refractColor, Kt)
    ...
    def rtRender(self):
        ...
        for i, j in indices:
            ...
            if 0<=x<self.width and 0<=y<self.height:
                ...
                direction_norm = np.custom_linalg_norm(direction)
                if direction_norm != 0:
                    direction = np.custom_multiply_vector_by_scalar(direction, 1 / direction_norm)
